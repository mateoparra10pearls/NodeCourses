...
@injectable()
class UserSectionController implements IBaseController {
  public path = "/userSections";
  public router = express.Router();
  private _service: IBaseService<UserSection>;

  constructor(@inject(TYPES.UserSectionService) service: IBaseService<UserSection>) {
  ...
  save = async (req: Request, res: Response): Promise<any> => {
    // Retrieve the UserSection from our URL path
    const data = <UserSection>req.body;
